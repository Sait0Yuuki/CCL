...
int64_t now()
{
	struct timeval tv; //在 _timeval.h 中声明了该结构体，两个成员变量 tv_sec（秒） 和 tv_usec（微秒）
	gettimeofday(&tv, NULL);
	printf("main.cc\n");
	printf("%d s\n", unsigned(tv.tv_sec + tv.tv_usec * 1000000));

	return tv.tv_sec * 1000000 + tv.tv_usec; //此处是不是反了？tv_sec（秒）？tv_usec（微秒）？
}
